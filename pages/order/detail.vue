  ...
  <view v-if="!isLoading" class="container">
    ...
    <view class="goods-list i-card">
      <view class="goods-item" v-for="(goods, idx) in order.goods" :key="idx">
        <view class="goods-main" @click="handleTargetGoods(goods.goods_id,order.order_source)">
          <!-- 商品图片 -->
          ...
          <view class="goods-trade">
            <view class="goods-price">
               <text class="value" v-if="goods.goods.pay_type==1 || goods.goods.pay_type==3">积分{{ goods.goods.goods_point_max }}</text>
              <text class="unit" v-if="goods.goods.pay_type==2 || goods.goods.pay_type==3">￥</text>
              <text class="value" v-if="goods.goods.pay_type==2 || goods.goods.pay_type==3">{{ goods.goods.goods_price_max }}</text>
    ...
    <view class="trade-info i-card">
      ...
      <view class="trade-total">
        ...
        <view class="goods-price">
          <text class="value" v-if="order.goods[0].goods.pay_type==1 || order.goods[0].goods.pay_type==3">积分{{ order. pay_point}}</text>
          <text class="unit" v-if="order.goods[0].goods.pay_type==2 || order.goods[0].goods.pay_type==3">￥</text>
          <text class="value" v-if="order.goods[0].goods.pay_type==2 || order.goods[0].goods.pay_type==3">{{ order.pay_price }}</text>
          <!-- <text class="unit">￥</text>
    ...
    <view v-if="order.order_status == OrderStatusEnum.NORMAL.value" class="footer-fixed">
      <view class="btn-wrapper">
        <block v-if="order.delivery_status == DeliveryStatusEnum.NOT_DELIVERED.value && order.order_source !=40">
