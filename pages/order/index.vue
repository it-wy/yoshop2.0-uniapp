            ...
            <view class="goods-item" v-for="(goods, idx) in item.goods" :key="idx">
              ...
              <view class="goods-trade">
                <view class="goods-price">
                  <text class="value" v-if="goods.pay_type==1 || goods.pay_type==3">积分{{ goods.total_point }}</text>
                  <text class="unit" v-if="goods.pay_type==2 || goods.pay_type==3">￥</text>
                  <text class="value" v-if="goods.pay_type==2 || goods.pay_type==3">{{ goods.goods_price }}</text>
                </view>
              ...
              <view class="order-total">
                <text>共{{ item.total_num }}件商品，总计</text>
                <text class="money" v-if="goods.pay_type==1 || goods.pay_type==3">积分{{ item.pay_point }}</text>
                <text class="unit" v-if="goods.pay_type==2 || goods.pay_type==3">￥</text>
                <text class="money" v-if="goods.pay_type==2 || goods.pay_type==3">{{ item.pay_price }}</text>
              </view>
            </view>
          </view>
          ...
          <view v-if="item.order_status != 20" class="order-handle">
            <view class="btn-group clearfix">
              ...
              <block v-if="item.order_status != OrderStatusEnum.APPLY_CANCEL.value">
                <view v-if="item.pay_status == PayStatusEnum.SUCCESS.value && item.delivery_status == DeliveryStatusEnum.NOT_DELIVERED.value && item.order_source !=40">
              ...
              <view v-if="item.pay_status == 10">
                <view class="btn-item active" @click="onPay(item.order_id,item)">去支付</view>
              </view>
              <!-- 确认收货 -->
    ...
    <u-popup v-model="showPayPopup" mode="bottom" border-radius="26" :closeable="true">
      <view class="pay-popup">
        ...
        <view class="pop-content">
		  ...
		  <!-- 积分支付 -->
		  <view class="pay-item dis-flex flex-x-between" v-if="order.pay_type == 1  && assets.points >=  Number(order.goods_point)" @click="onSelectPayType(PayTypeEnum.INTERGAL.value)">
		    <view class="item-left dis-flex flex-y-center">
		      <view class="item-left_icon balance">
          ...
          OrderApi.list({ dataType: app.getTabValue(), page: pageNo }, { load: false })
            .then(result => {
              ...
              if(app.list.data.length>0){
                  app.order = app.list.data[0].goods[0]
              }
