    ...
    <view v-if="!isLoading" class="goods-info m-top20">
      ...
      <view class="info-item info-item__top dis-flex flex-x-between flex-y-end">
        <view class="block-left dis-flex flex-y-end">
          <!-- 商品售价 -->
          <text class="floor-price" v-if="goods.pay_type==1 || goods.pay_type==3">积分 {{ goods.goods_point_min }}</text>
      ...
      <view class="footer-container">
        ...
        <view class="foo-item-btn">
          <view class="btn-wrapper">
            <view class="btn-item btn-item-deputy" v-if="paytxt.length==4" @click="onShowSkuPopup(2)">
              <text>加入购物车</text>
            </view>
...
<script>
  ...
  export default {
    ...
    onLoad(options) {
      ...
      this.onRefreshPage()
	  // 根据参数判断是否达人商品
	  this.paytxt = options.payType? '申请成为城市达人':this.paytxt;
	  // console.log( this.paytxt)
