<template>
  <mescroll-body ref="mescrollRef" :sticky="true" @init="mescrollInit" :down="{ native: true }" @down="downCallback"
    ...
    <view class="store-sort">
	  ...
	  <!-- <view class="sort-item" :class="{ active: sortType === 'brand' }" @click="handleSortType('brand')">
	    <text>品牌</text>
	  </view> -->
    </view>
	...
	<u-popup v-model="show" mode="right"  >
		<view class="container">
			<p class="brand_title">品牌  <u-icon  name="arrow-up" :class="{tops: isbrand,bottoms: !isbrand}" ref="arrowup" @click="isbrand=!isbrand"></u-icon></p>
			<view class="popup_brand" v-if="isbrand">
				<view v-for="(item,i) in brandList" :key="i" class="brand-item" :class="{acv: idx==i}" @click="onSelect(i)">
					{{item}}
				</view>
			</view>
			<!-- 类别 -->
			<p class="brand_title">类别  <u-icon  name="arrow-up" :class="{tops: iscategory,bottoms: !iscategory}"  @click="iscategory=!iscategory"></u-icon></p>
			<view class="popup_brand" v-if="iscategory">
				<view class="brand-item acv">
					KFC
				</view>
			...
			<!-- 快速指引 -->
			<p class="brand_title">快速指引</p>
			<view class="popup_brand">
				<view class="brand-item acv">
					品牌热卖
				</view>
				<view class="brand-item">
      ...
      return {
        ...
        upOption: {
          ...
          noMoreSize: 4,
		    ...
		    show: false, // 品牌弹窗,
        brandList: ['特朗普','拜等','管易伟','安倍晋三'], // 品牌列表
        idx: 0,
        isbrand: false,
        iscategory: false,
      }
...
<style lang="scss" scoped>
  ...
  .action_btn{
    ...
    button { 
      ...
      border-radius: 0;
      &:first-child{
         border: 1px solid rgba(203, 197, 197, 0.33);
        background: #FFFFFF;
      }
      &:last-child {
        background: #FF903F;
