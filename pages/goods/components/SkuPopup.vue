<template>
  <goods-sku-popup :value="value" @input="onChangeValue" border-radius="20" :custom-action="findGoodsInfo"
    ...
    @open="openSkuPopup" @close="closeSkuPopup" @add-cart="addCart" @buy-now="buyNow" :maxBuyNum="maxBuyNum" :buyNowText="buyNowText" />
</template>
...
<script>
  ...
  export default {
    ...
    created() {
      const app = this
      const { goods } = app
      if(goods.attr==5) {app.maxBuyNum = 1;app.buyNowText = '同意协议'}
