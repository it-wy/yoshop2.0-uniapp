  ...
  <view class="container" >
        ...
        <view class="item-content">
          ...
          <view class="item-foot">
            <view class="goods-price">
              <view class="value" v-if="item.goods.pay_type==1 || item.goods.pay_type==3">积分 {{ item.goods.goods_point_min}}</view>
              <text class="unit" v-if="item.goods.pay_type==2 || item.goods.pay_type==3">￥</text>
              <text class="value" v-if="item.goods.pay_type==2 || item.goods.pay_type==3">{{ item.goods.skuInfo.goods_price }}</text>
            </view>
            <view class="stepper">
    ...
    <view v-if="list.length" class="footer-fixed">
      ...
      <view class="total-info">
        <text>合计：</text>
        <view class="goods-price">
          <text class="value">积分{{ allpoint }}</text>
    ...
    methods: {
      ...
      onCalcTotalPrice() {
        ...
        checkedList.forEach(item => {
          ...
          const unitPrice = item.goods.pay_type==2 || item.goods.pay_type ==3 ? item.goods.skuInfo.goods_price * 100: 0;
          tempPrice += unitPrice * item.goods_num
          
          // 积分计算
          const unitPoint= item.goods.pay_type==1 || item.goods.pay_type ==3 ? item.goods.skuInfo.goods_point * 100: 0;
          tempPoint += unitPoint*item.goods_num;
          
        })
        
        app.totalPrice = (tempPrice / 100).toFixed(2)
        app.allpoint = (tempPoint / 100).toFixed(2)
      },
      ...
      handleOrder() {
        const app = this
        if (app.checkedIds.length>0) {
          // 循环收集类型
          const array = []
          app.checkedIds.map((k)=>{
              app.list.map((v)=>{
                if(k==v.id){
                  array.push(v.goods.pay_type)
                }
              });
          })
