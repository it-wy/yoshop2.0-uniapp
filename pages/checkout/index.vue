<template>
  <view class="container p-bottom" v-if="order.goodsList.length">
        ...
        <view class="flow-shopList dis-flex" data-index="index" @click="onTargetGoods(item.goods_id)">
          ...
          <view class="flow-list-right flex-box">
            ...
            <view class="flow-list-cont dis-flex flex-x-between flex-y-center">
              ...
              <text class="flow-cont" v-if="item.pay_type==1 || item.pay_type==3">积分 {{ item.goods_point_min }}</text>
              <text class="flow-cont" v-if="item.pay_type==2 || item.pay_type==3">￥{{ item.goods_price }}</text>
    ...
    <view class="pay-method flow-all-money b-f m-top20">
      ...
      <view v-if="order.goodsList[0].pay_type==1 ? false: order.goodsList[0].pay_type == 2? true :assets.points >=  Number(order.goodsList[0].goods_point_min)? true: false" class="pay-item dis-flex flex-x-between" @click="handleSelectPayType(PayTypeEnum.WECHAT.value)">
      ...
      <!-- 余额支付 -->
      <view v-if="order.goodsList[0].pay_type==1 ? false: order.goodsList[0].pay_type == 2? true :assets.points >=  Number(order.goodsList[0].goods_point_min)? true: false" class="pay-item dis-flex flex-x-between" @click="handleSelectPayType(PayTypeEnum.BALANCE.value)">
        <view class="item-left dis-flex flex-y-center">
	  ...
	  <!-- 积分支付 -->
	  <view v-if="order.goodsList[0].pay_type == 1  && assets.points >=  Number(order.goodsList[0].goods_point_min)" class="pay-item dis-flex flex-x-between" @click="handleSelectPayType(PayTypeEnum.INTERGAL.value)">
	    <view class="item-left dis-flex flex-y-center">
	      <view class="item-left_icon balance">
	        <text class="iconfont icon-jifen"></text>
	      </view>
	      <view class="item-left_text">
	        <text>{{ PayTypeEnum.INTERGAL.name }}</text>
	      </view>
	    </view>
	    <view class="item-right col-m" v-if="curPayType == PayTypeEnum.INTERGAL.value">
	      <text class="iconfont icon-duihao"></text>
	    </view>
	  </view>
    <view v-if="order.goodsList[0].pay_type==1 || order.goodsList[0].pay_type==3">
      <view v-if="assets.points <=  order.goodsList[0].goods_point_min" style="color:#f50;">当前用户积分不足</view>
      </view>
    </view>
    ...
    <view class="flow-fixed-footer b-f m-top10">
      <view class="dis-flex chackout-box">
        <view class="chackout-left pl-12">实付款：
          <!-- <text class="col-m" v-if="order.goodsList[0].pay_type == 1  && assets.points >=  Number(order.goodsList[0].goods_point_min)">积分 {{ Number(order.goodsList[0].goods_point_min)* order.goodsList[0].total_num}}</text> -->
          <!-- <text class="col-m" v-if="order.goodsList[0].pay_type==1 ? false: order.goodsList[0].pay_type == 2? true :assets.points >=  Number(order.goodsList[0].goods_point_min)? true: false">￥{{ order.orderPayPrice }}</text> -->
          <text class="col-m" v-if="order.goodsList[0].pay_type!=1">{{order.orderTotalPrice}}</text>
  ...
  export default {
    data() {
      return {
        ...
        curDelivery: null,
        // 当前选中的支付方式
        curPayType: PayTypeEnum.INTERGAL.value,
        // 选择的优惠券
        selectCouponId: 0,
        // 是否使用积分抵扣
        isUsePoints: false,
    ...
    methods: {
      ...
      initData({ order }) {
        ...
        if(app.order.goodsList[0].pay_type==1 || app.order.goodsList[0].pay_type==3){
          if(app.assets.points <=  app.order.goodsList[0].goods_point_min){
            app.disabled = true;
          }
        }
        ...
        // }
        if(order.goodsList[0].pay_type == 2){
            app.curPayType = PayTypeEnum.BALANCE.value
        }

        if(order.goodsList[0].pay_type == 3){

          if(app.assets.points >=  Number(order.goodsList[0].goods_point_min)){
                app.curPayType = PayTypeEnum.BALANCE.value
          }
        }
    },
      ...
      onSubmitCallback(result) {
		...
		// 积分支付
		if (result.data.payType == PayTypeEnum.INTERGAL.value) {
		  app.$success('支付成功')
		  app.disabled = false
		  app.navToMyOrder()
		}
      ...
      getFormData() {
        ...
        // 创建订单-立即购买
        if (options.mode === 'buyNow') {
          form.goodsId = Number(options.goodsId)
